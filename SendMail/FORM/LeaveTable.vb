...
#Region "procedure"
    ...
    End Sub
    Sub editData()
        If UCase(getDataGridView(dg, "status")) = "APPROVED" Then
            MsgError("Cuti Sudah Di Approved, Tidak Bisa Edit Cuti")
            Exit Sub
        End If
        LeaveForm.dtStartdate.Focus()
        LeaveForm.txtNIP.Enabled = False
        LeaveForm.BtnCariNIP.Enabled = False
        LeaveForm.lblTipe.Text = "edit"
        With LeaveForm
            .isiCboLeaveType()
            .txtNIP.Text = getDataGridView(dg, "nip")
            .dtStartdate.Value = getDataGridView(dg, "startdate")
            .dtEnddate.Value = getDataGridView(dg, "enddate")
            .dtLeaveSource.Value = getDataGridView(dg, "tdate")
            .txtLeaveTypeID.Text = getDataGridView(dg, "leavetype_id")
            .txtLeaveType.Text = getStringFromDB("select ltrim(rtrim(code))+'-'+ltrim(rtrim(name)) as aa " & vbCrLf & _
                               "from m_leavetype " & vbCrLf & _
                               "where leavetype_id='" & .txtLeaveTypeID.Text & "' " & vbCrLf & _
                               "order by ltrim(rtrim(code))+'-'+ltrim(rtrim(name)) ")
            .txtTakenLeave.Text = getDataGridView(dg, "takenleave")
            .txtDescription.Text = getDataGridView(dg, "description")
            .txtStatus.Text = getDataGridView(dg, "status")
            .txtAvailableLeave.Text = getDataGridView(dg, "availableleave")
            .txtEmployeeID.Text = getDataGridView(dg, "employee_id")
            .txtEmployeeCutiID.Text = getDataGridView(dg, "employee_cuti_id")
            .txtTCutiID.Text = getDataGridView(dg, "tcuti_id")
            .cbxCutiTahunan.Checked = False
            If CInt(getDataGridView(dg, "iscutitahunan")) = 1 Then .cbxCutiTahunan.Checked = True
        End With
    ...
    Sub addData()
        LeaveForm.Bersih()
        LeaveForm.txtNIP.Focus()
        LeaveForm.txtNIP.Enabled = True
        LeaveForm.BtnCariNIP.Enabled = True
        LeaveForm.lblTipe.Text = "new"
        LeaveForm.txtStatus.Text = "ENTRY"
        LeaveForm.ShowDialog()
        OpenDBHRD()
        reloadClick()
        txtSPG_Barcode.Clear()
    ...
    Sub DeleteData()
        ...
        If MsgConfirm("Anda Yakin Ingin Hapus Data " & dg.SelectedRows.Count & " Record ?") = MsgBoxResult.Yes Then
            For Each row As DataGridViewRow In dg.SelectedRows
                'Dbg("delete from t_cuti where tcuti_id= '" & row.Cells("tcuti_id").Value & "' ")
                If UCase(getDataGridView(dg, "status")) <> "APPROVED" Then
                    'MsgError("Cuti Sudah Di Approved, Tidak Bisa Edit Cuti")
                    'Exit Sub
                    ExecQuery("delete from t_cuti where tcuti_id= '" & row.Cells("tcuti_id").Value & "' ", StrConHRD)
                End If
            Next
            reloadClick()
        End If
...
#Region "button"
    ...
    Private Sub BtnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnEdit.Click
        'For x As Integer = 0 To dg.ColumnCount - 1
        '    MsgOK(dg.Columns(x).DataPropertyName)
        'Next
        editData()
    End Sub
    ...
    End Sub
    Private Sub BtnConsignment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'TampilDirect("xx")
...
#End Region
    ...
    Private Sub BtnAktif_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        TampilTable()
        FilterTextDefault()
        txtSPG_Barcode.Focus()
    End Sub
    ...
    Private Sub ApprovedToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ApprovedToolStripMenuItem.Click
        If MsgConfirm("Anda Yakin Ingin Approved " & dg.SelectedRows.Count & " Record ?") = MsgBoxResult.Yes Then
            For Each row As DataGridViewRow In dg.SelectedRows
                If UCase(row.Cells("status").Value) <> "APPROVED" Then
                    UbahStatusCuti(row.Cells("tcuti_id").Value, row.Cells("employee_id").Value, "APPROVED")
                End If
            Next
            reloadClick()
        End If
    ...
    Private Sub CancelApprovedToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CancelApprovedToolStripMenuItem.Click
        If MsgConfirm("Anda Yakin Ingin Cancel Approved " & dg.SelectedRows.Count & " Record ?") = MsgBoxResult.Yes Then
            For Each row As DataGridViewRow In dg.SelectedRows
                If UCase(row.Cells("status").Value) = "APPROVED" Then
                    UbahStatusCuti(row.Cells("tcuti_id").Value, row.Cells("employee_id").Value, "ENTRY")
                End If
            Next
            reloadClick()
        End If
    End Sub
    Sub UbahStatusCuti(ByVal tcuti_id As String, ByVal eid As String, ByVal status As String)
        Dim kolom As String
        ...
        Else
            kolom = "usercancel"
        End If
        ExecQuery("update t_cuti set status='" & status & "', useredited='" & UserLogin & "', " & vbCrLf & _
                  "dateedited='" & Format(Now(), "yyyy-MM-dd hh:mm:ss") & "', " & vbCrLf & _
                  " " & kolom & "='" & UserLogin & "' where tcuti_id='" & tcuti_id & "' ")
        isiMasterCuti(eid)
    End Sub
End Class